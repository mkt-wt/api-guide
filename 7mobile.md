...
#### 주의사항
**1) 본 코드의 '요금제' 부분은 MVNO사업혁신팀 현업에서 내용 확인 후 변경될 수 있습니다.**
2) 로그인하는 사용자의 회원정보 중 성별, 연령, 요금제 정보를 참조해서 값을 입력해야 합니다.
3) `성별코드`, `연령대코드`에는 아래 표에 있는 코드 값을 넣어주시고, `현재요금제명칭`, `기존요금제명칭`에는 텍스트로 표현된 명칭 그대로를 입력해 주세요.
